...
步骤说明：
  ...
  step6: 缓冲情景 MRIO 路径份额版（chokefrac 缩放 v_share_trade，要道间精确区分）

注意：step2 / step5 / step6 均需加载 EORA（约 1–2 分钟）。
      完整流程（run_full）只加载一次，两步共用同一份缓存。
      step6 不在 run_full 默认流程中，需用 --step 6 单独运行。
...
from phase3_io_analysis import (  # noqa: E402
    ...
    step6_buffer_scenarios_pathfrac,
)
...
def run_single_step(step: int, directions: list[str]) -> None:
    ...
    if step == 1:
        ...
        eora_data = step6_buffer_scenarios_pathfrac.load_eora_once()
        for d in directions:
            step6_buffer_scenarios_pathfrac.run(d, eora_data=eora_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="只运行指定步骤编号（1–6；6 = 路径份额版缓冲情景）",
    )
    parser.add_argument(
